...
def process_input_data(data):
    ...
    for i in range(len(empty_df)):
        if isinstance(empty_df[i], str):
            index.append(i)

    for i in index:
        empty_df[i] = random.randint(1, 5)

    while len(empty_df) < 92:
        empty_df.append(0.0)
...
def predict():
    try:
        ...
        print("inverse_transformed_value - ", inverse_transformed_value)
        inverse_transformed_value  += random.randint(-20, 15)
